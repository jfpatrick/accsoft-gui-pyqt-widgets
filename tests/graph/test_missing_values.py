...
def test_invalid_curves(qtbot, minimal_test_window, warn_always, missing_value):
    ...
    with pytest.warns(accgraph.InvalidDataStructureWarning):
        ds.sig_data_update[accgraph.PointData].emit(accgraph.PointData(x_value=missing_value, y_value=2.0))
    buffer: accgraph.SortedCurveDataBuffer = minimal_test_window.plot.plotItem.get_live_data_curves()[0].get_data_model().get_full_data_buffer()
    np.testing.assert_equal(buffer[0], [])
    np.testing.assert_equal(buffer[1], [])
    # valid
    ds.sig_data_update[accgraph.PointData].emit(accgraph.PointData(x_value=0.0, y_value=3.0))
    buffer: accgraph.SortedCurveDataBuffer = minimal_test_window.plot.plotItem.get_live_data_curves()[0].get_data_model().get_full_data_buffer()
    np.testing.assert_equal(buffer[0], [0.0])
    np.testing.assert_equal(buffer[1], [3.0])
    # invalid
    with pytest.warns(accgraph.InvalidDataStructureWarning):
        ds.sig_data_update[accgraph.PointData].emit(accgraph.PointData(x_value=missing_value, y_value=4.0))
    buffer: accgraph.SortedCurveDataBuffer = minimal_test_window.plot.plotItem.get_live_data_curves()[0].get_data_model().get_full_data_buffer()
    np.testing.assert_equal(buffer[0], [0.0])
    np.testing.assert_equal(buffer[1], [3.0])
...
def test_valid_bars(qtbot, minimal_test_window, recwarn, warn_always, missing_value):
    ...
    model: accgraph.BarGraphDataModel = minimal_test_window.plot.plotItem.get_live_data_bar_graphs()[0].get_data_model()
    # valid
    ds.sig_data_update[accgraph.BarData].emit(accgraph.BarData(x_value=1.0, y_value=missing_value, height=2.0))
    assert len(recwarn) == 0
    buffer = model.get_full_data_buffer()
    ...
    # valid
    ds.sig_data_update[accgraph.BarData].emit(accgraph.BarData(x_value=0.0, y_value=1.0, height=2.0))
    assert len(recwarn) == 0
    buffer = model.get_full_data_buffer()
    np.testing.assert_equal(buffer[0], [0.0, 1.0])
...
def test_invalid_bars(qtbot, minimal_test_window, warn_always, missing_value):
    ...
    # invalid
    with pytest.warns(accgraph.InvalidDataStructureWarning):
        ds.sig_data_update[accgraph.BarData].emit(accgraph.BarData(x_value=1.0, y_value=1.0, height=missing_value))
    buffer = model.get_full_data_buffer()
    np.testing.assert_equal(buffer[0], [])
    np.testing.assert_equal(buffer[1], [])
    ...
    # invalid
    with pytest.warns(accgraph.InvalidDataStructureWarning):
        ds.sig_data_update[accgraph.BarData].emit(accgraph.BarData(x_value=missing_value, y_value=1.0, height=1.0))
    buffer = model.get_full_data_buffer()
    np.testing.assert_equal(buffer[0], [1.0])
    np.testing.assert_equal(buffer[1], [0.0])
    np.testing.assert_equal(buffer[2], [2.0])
...
def test_valid_injection_bars(qtbot, minimal_test_window, warn_always, recwarn, missing_value):
    ...
    model: accgraph.InjectionBarDataModel = minimal_test_window.plot.plotItem.get_live_data_injection_bars()[0].get_data_model()
    # valid
    ds.sig_data_update[accgraph.InjectionBarData].emit(
...
def test_valid_timestamp_markers(qtbot, minimal_test_window, warn_always, recwarn, missing_value):
    ...
    model: accgraph.TimestampMarkerData = minimal_test_window.plot.plotItem.get_timestamp_markers()[0].get_data_model()
    # valid
    ds.sig_data_update[accgraph.TimestampMarkerData].emit(
