...
class RbaButtonModel(QObject):
    ...
    @Slot(bytes)
    @Slot(str)
    def update_token(self, new_token: Union[Token, bytes, str]):
        """
        ...
        .. note:: When injecting external token, you will lose certain features, such as proper roles display.
                  Available roles are usually only provided temporarily during login process, when specifically
                  requested, and it is not possible to derive a complete set of roles from the existing token.
