...
from qtpy.QtWidgets import QDialog, QWidget, QDialogButtonBox, QPushButton, QCheckBox, QListView, QLabel
...
class RbaCriticalRolesModel(QSortFilterProxyModel):
    ...
    def filterRole(self) -> int:
        return RbaRolesModel.CRITICAL_ROLE
...
class RbaRolesModel(QAbstractListModel):

    CRITICAL_ROLE = Qt.UserRole + 11
    """Role for the "critical" flag."""

    def __init__(self, data: List[RbaRole], parent: Optional[QObject] = None):
        """
    ...
    def data(self, index: QModelIndex, role: Qt.ItemDataRole = Qt.DisplayRole) -> Any:
        ...
        if role == Qt.CheckStateRole:
            ...
            is_critical = index.data(self.CRITICAL_ROLE)
            if is_critical:
                return QBrush(get_color(self, ColorRole.MCS))  # Else handle by default
...
class RbaRolePicker(QDialog):
    ...
    def __init__(self,
                 roles: List[RbaRole],
                 display_auto_renewable_notice: bool = False,
        ...
        Args:
            ...
            display_auto_renewable_notice: Display notice that choosing roles will cancel auto-renewable feature.
        ...
        if not display_auto_renewable_notice:
            self.notice_label.hide()
        self.notice_label.setProperty("qss-role", "info")
        ...
        self._src_model = RbaRolesModel(data=roles, parent=self)
        self._model = RbaCriticalRolesModel(self)
...
def select_roles_interactively(roles: List[RbaRole], parent: Optional[QWidget] = None) -> List[str]:
    """
    Method that displays a role picker dialog to pick the roles desired by the user.

    This piece of GUI is not connected to anything, and is intended to be displayed in the callback of
    :mod:`pyrbac`, when it requests the roles to be picked during the login process.

    Args:
        roles: List of all available role objects.
        parent: Owning object of the picker.

    Returns:
        The names of the selected roles.
    """
    picker = RbaRolePicker(roles=roles, display_auto_renewable_notice=False, force_select=True, parent=parent)
    res: List[str] = []
