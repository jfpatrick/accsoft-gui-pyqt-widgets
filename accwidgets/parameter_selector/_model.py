...
class AbstractNestedStringListModel(QStringListModel, metaclass=GenericQtMeta):
    ...
    def root_item_selection_changed(self, index: Union[int, QModelIndex]):
        ...
        if idx < 0:
            logger.warning("In Parameter Selector, an invalid root item is selected. This should not happen!")
            return
    ...
    def intermediate_item_selection_changed(self, index: Union[int, QModelIndex]):
        ...
        if self._selected_root < 0:
            logger.warning("In Parameter Selector, an intermediate item is selected while root is undefined. This should not happen!")
            return
    ...
    def leaf_selection_changed(self, index: Union[int, QModelIndex]):
        ...
        if self._selected_root < 0:
            # FIXME: Use warnings
            logger.warning("In Parameter Selector, a leaf item is selected while root is undefined. This should not happen!")
            return
...
class DeviceListModel(AbstractNestedStringListModel):
    ...
    def result(self) -> str:
        if self._selected_root < 0 or self._selected_root >= len(self._items):
            return ""
        item = self._items[self._selected_root]
        result = ParameterName(device=item.name, prop="")
        if self._selected_intermediate_node >= 0:
            sub_item = item.children[self._selected_intermediate_node]
            result.prop = sub_item.name
...
async def _look_up_ccda(device_name: str, searched_prop: Optional[str], searched_field: Optional[str]) -> List[NestedListRootItem]:
